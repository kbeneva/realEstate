...
function AgentRequestList() {
    ...
    const loadRequests = async () => {
        try {
            ...
            setRequests(allRequests.filter(request => request.statusDemande === "pending" && request[`property${request.typeDemande}`].accepted === true)); // only showing what hasn't been treated and only properties that are still in the store
